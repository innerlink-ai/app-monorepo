    ...
    <div class="flex-1 flex overflow-hidden bg-[var(--bg-color)]">
      <div class="max-w-7xl w-full mx-auto p-6 overflow-y-auto">
        ...
        <div class="mb-8 border border-[var(--border-color)] rounded-lg p-6 bg-[var(--bg-color)]">
          <h2 class="text-sm font-semibold mb-4">Invite New User</h2>

          <!-- Fields (Side by Side on md+, stacked on small screens) -->
          <div class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
              <label for="email" class="block text-xs font-medium mb-1 text-[var(--secondary-text)]">Email Address</label>
              <input 
                id="email"
                v-model="email"
                type="email"
            ...
            <div class="w-full md:w-auto">
              <label for="role" class="block text-xs font-medium mb-1 text-[var(--secondary-text)]">Access Role</label>
              <select 
                id="role"
                v-model="role"
          ...
          <!-- Invite Button & Error/Success Messages -->
          <div class="mt-6 flex flex-col md:flex-row items-center justify-between">
            ...
            <div class="mt-2 md:mt-0 text-right">
              <p v-if="serverError" class="text-red-500 text-xs">{{ serverError }}</p>
              <p v-if="inviteMessage" class="text-xs text-green-600">{{ inviteMessage }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="border border-[var(--border-color)] rounded-lg overflow-hidden">
          <div class="p-4 border-b border-[var(--border-color)] bg-[var(--bg-color)]">
            <h2 class="text-sm font-semibold">Pending Invites</h2>
          </div>
          <div class="overflow-auto max-h-[250px] bg-[var(--bg-color)]">
            <table class="w-full text-left border-collapse text-sm">
              <thead>
                <tr class="border-b border-[var(--border-color)]">
                  <th class="py-3 px-4 font-medium text-[var(--secondary-text)]">Email</th>
                  <th class="py-3 px-4 font-medium text-[var(--secondary-text)]">Role</th>
                  <th class="py-3 px-4 font-medium text-[var(--secondary-text)]">Invited Date</th>
...
const checkAuth = async () => {
  try {
    ...
    if (isAuthenticated.value) {
      ...
      // Optionally redirect if auth fails strictly
      // router.push('/login');
  }
};
...
const handleInviteUser = async () => {
  ...
  // Create the payload according to InvitePayload type
  // Assuming it expects email and is_admin
  const payload: InvitePayload = {
    email: email.value,
    is_admin: role.value === 'admin' // Convert role string to boolean
  };
  ...
  try {
    ...
    email.value = "";
    role.value = "user"; // Reset dropdown

    await fetchAllData(); // Refresh pending invites
  } catch (error: any) {
    serverError.value = error.message || "Failed to send invitation.";
    console.error(error);
  }
};
